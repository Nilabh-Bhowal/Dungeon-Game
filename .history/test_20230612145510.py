...
def collide(player, rooms):
    for room in rooms:
        if player.rect.colliderect(room):
            ...
            pu = any(
                player.rect.y - player.rect.height < space.rect.y + space.rect.height
                and player.rect.x < space.rect.x
                and player.rect.x + player.rect.width > space.rect.x + space.rect.width
                for space in rooms
            )
            pd = any(
                player.rect.y + 2 * player.rect.height > space.rect.y
                and player.rect.x < space.rect.x
                and player.rect.x + player.rect.width > space.rect.x + space.rect.width
                for space in rooms
            )
            pl = any(
                player.rect.x - player.rect.width < space.rect.x + space.rect.width
                and player.rect.y < space.rect.y
                and player.rect.y + player.rect.height > space.rect.y + space.rect.height
                for space in rooms
            )
            pr = any(
                player.rect.x + 2 * player.rect.width > space.rect.x
                and player.rect.y < space.rect.y
                and player.rect.y + player.rect.height > space.rect.y + space.rect.height
                for space in rooms
            )
...
player = Player()
dungeons = [dungeon.DungeonRoom(128, -152), dungeon.DungeonRoom(1152, -152)]
