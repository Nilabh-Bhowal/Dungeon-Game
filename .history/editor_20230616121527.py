import pygame

import dungeon
import ui

pygame.init()

display = pygame.display.set_mode((1280, 720))
screen = pygame.Surface((640, 360))


def save(num, level):
    with open(f'levels/{num}.txt', 'w') as f:
        for item in level:
            if isinstance(item, dungeon.DungeonRoom):
                l = 0
            elif isinstance(item, dungeon.Corridor):
                l = 1
            elif isinstance(item, dungeon.Chest):
                l = 2
            f.write(f"[{l}, {item.rect.x}, {item.rect.y}]\n")


def load(num):
    with open(f"levels/{num}.txt", "r") as f:
        items = (f.read().splitlines())
        items = [eval(item) for item in items]
        level = []
        for item in items:
            if item[0] == 0:
                level.append(dungeon.DungeonRoom(item[1], item[2]))
            elif item[0] == 1:
                level.append(dungeon.Corridor(item[1], item[2]))
            elif item[0] == 2:
                level.append(dungeon.Chest(item[1], item[2]))

    return level


def check_collision(placed, level, scroll):
    for item in level:
        if placed != item and item.rect.colliderect(placed.rect) and not isinstance(placed, dungeon.Chest) and not isinstance(item, dungeon.Chest):
            if placed.rect.bottom > item.rect.top and (pygame.mouse.get_pos()[1] + scroll[1]) / 2 < item.rect.top:
                placed.rect.bottom = item.rect.top
            elif placed.rect.top < item.rect.bottom and (pygame.mouse.get_pos()[1] + scroll[1]) / 2 > item.rect.bottom:
                placed.rect.top = item.rect.bottom
            if placed.rect.right > item.rect.left and (pygame.mouse.get_pos()[0] + scroll[0]) / 2 < item.rect.left:
                placed.rect.right = item.rect.left
            elif placed.rect.left < item.rect.right and (pygame.mouse.get_pos()[0] + scroll[0]) / 2 > item.rect.right:
                placed.rect.left = item.rect.right


scroll = [0, 0]
cam_movement = [0, 0]

level = []
current_item = "room"

pressed = False

clock = pygame.time.Clock()

room_button = ui.Button("Comic Sans MS", 8, (255, 255, 0),
                        "Dungeon Room", 500, 100, 100, 25)
corridor_button = ui.Button("Comic Sans MS", 8, (255, 255, 0),
                            "Corridor", 500, 150, 100, 25)
chest_button = ui.Button("Comic Sans MS", 8, (255, 255, 0),
                         "Chest", 500, 200, 100, 25)

running = True
while running:

    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RIGHT:
                cam_movement[0] = 1
            if event.key == pygame.K_LEFT:
                cam_movement[0] = -1
            if event.key == pygame.K_DOWN:
                cam_movement[1] = 1
            if event.key == pygame.K_UP:
                cam_movement[1] = -1

            if event.key == pygame.K_s:
                save(0, level)
            if event.key == pygame.K_l:
                level = load(0)

        if event.type == pygame.KEYUP:
            if event.key in [pygame.K_RIGHT, pygame.K_LEFT]:
                cam_movement[0] = 0
            if event.key in [pygame.K_UP, pygame.K_DOWN]:
                cam_movement[1] = 0

    if pygame.mouse.get_pos()[0] / 2 <= 490 and pygame.mouse.get_pressed()[0] and not pressed:
        pressed = True
        if current_item == "room":
            level.append(dungeon.DungeonRoom(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 256) / 32) * 32, round((pygame.mouse.get_pos()[1] + scroll[1] - 256) / 32) * 32))
        elif current_item == "corridor":
            level.append(dungeon.Corridor(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 64) / 32) * 32, round((pygame.mouse.get_pos()[1] + scroll[1] - 32) / 32) * 32))
        elif current_item == "chest":
            level.append(dungeon.Chest(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 32) / 32) * 32, round((pygame.mouse.get_pos()[1] + scroll[1] - 16) / 32) * 32))
    if not pygame.mouse.get_pressed()[0]:
        pressed = False

    if pygame.mouse.get_pos()[0] / 2 <= 490 and pygame.mouse.get_pressed()[2]:
        for room in level:
            if room.rect.collidepoint((pygame.mouse.get_pos()[0] + scroll[0], pygame.mouse.get_pos()[1] + scroll[1])):
                level.remove(room)

    for item in reversed(level):
        check_collision(item, level, scroll)

    scroll[0] += cam_movement[0] * 10
    scroll[1] += cam_movement[1] * 10

    screen.fill((255, 100, 100))
    for room in level:
        room.draw(screen, scroll)
    for i in range(17):
        pygame.draw.line(screen, (0, 0, 0), (i * 32 -
                         scroll[0] % 32, 0), (i * 32 - scroll[0] % 32, 360))
    for i in range(13):
        pygame.draw.line(screen, (0, 0, 0), (0, i * 32 -
                         scroll[1] % 32), (490, i * 32 - scroll[1] % 32))
    pygame.draw.rect(screen, (0, 0, 0), (490, 0, 150, 360))

    if room_button.draw(screen):
        current_item = "room"
    if corridor_button.draw(screen):
        current_item = "corridor"
    if chest_button.draw(screen):
        current_item = "chest"

    display.blit(pygame.transform.scale(screen, (1280, 720)), (0, 0))
    pygame.display.update()
    clock.tick(60)

pygame.quit()
