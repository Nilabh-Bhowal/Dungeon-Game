...
while running:
    ...
    if pygame.mouse.get_pos()[0] <= 980 and pygame.mouse.get_pressed()[0] and not pressed:
        ...
        if current_item == "corridor":
            level.append(["corridor", pygame.mouse.get_pos()[0] + scroll[0], pygame.mouse.get_pos()[1] + scroll[1]])
        elif current_item == "room":
            level.append(["room", pygame.mouse.get_pos()[0] + scroll[0], pygame.mouse.get_pos()[1] + scroll[1]])
    ...
    if pygame.mouse.get_pos()[0] <= 980 and pygame.mouse.get_pressed()[2]:
        for room in level:
            if room[0] == "corridor":
                if room[1] < pygame.mouse.get_pos()[0] < room[1] + 256 and room[2] < pygame.mouse.get_pos()[1] < room[2] + 128:
                    room
