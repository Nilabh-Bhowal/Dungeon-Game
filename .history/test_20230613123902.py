...
def collide(player, rooms):
    for room in rooms:
        if player.rect.colliderect(room):
            ...
            if not pl:
                player.rect.x = max(player.rect.x, room.rect.x + 5)
                player.speed = 0
            if not pr:
                player.rect.x = min(player.rect.x, room.rect.x + room.rect.width - player.rect.width - 5)
                player.speed = 0
            if not pu:
                player.rect.y = max(player.rect.y, room.rect.y + 5)
                player.speed = 0
            if not pd:
                player.rect.y = min(player.rect.y, room.rect.y + room.rect.height - player.rect.height - 5)
                player.speed = 0
