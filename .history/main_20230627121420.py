...
class Enemy:
    ...
    def move(self, player, rooms):
        for room in rooms:
            if player.rect.colliderect(room) and self.rect.colliderect(room.rect)
        self.rect.x += self.speed * self.movement[0]
        self.rect.y += self.speed * self.movement[1]
        collide(self, rooms)

    def draw(self, screen, scroll):
        pygame.draw.rect(screen, (50, 75, 43), (self.rect.x - scroll[0], self.rect.y - scroll[1], self.rect.width, self.rect.height))
...
class Sword:
    ...
    def update(self):  # sourcery skip: hoist-statement-from-if, switch
