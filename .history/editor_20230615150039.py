...
def check_collision(rect, level, scroll):
    for item in level:
        print ((item.rect.top < pygame.mouse.get_pos()[1] + scroll[1] or item.rect.bottom > pygame.mouse.get_pos()[1] + scroll[1]))
        if rect != item.rect and item.rect.colliderect(rect) and (item.rect.top < pygame.mouse.get_pos()[1] + scroll[1] or item.rect.bottom > pygame.mouse.get_pos()[1] + scroll[1]):
            rect.top = max(rect.top, item.rect.bottom)
            rect.bottom = min(rect.bottom, item.rect.top)
            print("b")
        if rect != item.rect and item.rect.colliderect(rect) and (item.rect.left < pygame.mouse.get_pos()[0] + scroll[0] or item.rect.right > pygame.mouse.get_pos()[0] + scroll[0]):
            rect.left = max(rect.left, item.rect.right)
    ...
    if pygame.mouse.get_pos()[0] <= 980 and pygame.mouse.get_pressed()[0] and not pressed:
        ...
        if current_item == "room":
            level.append(dungeon.DungeonRoom((pygame.mouse.get_pos()[
                         0] + scroll[0]) // 64 * 64 - 512, pygame.mouse.get_pos()[1] + scroll[1] // 64 * 64 - 512))
        elif current_item == "corridor":
            level.append(dungeon.Corridor(pygame.mouse.get_pos()[
                         0] + scroll[0] // 64 * 64 - 128, pygame.mouse.get_pos()[1] + scroll[1] // 64 * 64 - 64))
