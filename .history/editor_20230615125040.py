...
while running:
    ...
    if pygame.mouse.get_pos()[0] <= 980 and pygame.mouse.get_pressed()[2]:
        for room in level:
            if room[0] == 0 and room[1] < pygame.mouse.get_pos()[0] + scroll[0] < room[1] + 1024 and room[2] < pygame.mouse.get_pos()[1] + scroll[1] < room[2] + 128:
                level.remove(room)
            if room[0] == 1 and room[1] < pygame.mouse.get_pos()[0] + scroll[0] < room[1] + 1024 and room[2] < pygame.mouse.get_pos()[1] + scroll[1] < room[2] + 1024:
                level.remove(room)
