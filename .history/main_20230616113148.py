...
def can_pass(player, current_room, rooms):
    ...
    for room in rooms:
        if room != current_room:
            pu = pu if pu else player.rect.top - player.rect.height * 2 < room.rect.bottom \
                                 and player.rect.top > room.rect.top \
                                 and player.rect.left >= room.rect.left \
                                 and player.rect.right <= room.rect.right
            pd = player.rect.bottom + player.rect.height * 2 > room.rect.top \
                and player.rect.bottom < room.rect.bottom\
                and player.rect.left >= room.rect.left \
                and player.rect.right <= room.rect.right if not pd else pd
            pl = player.rect.left - player.rect.width * 2 < room.rect.right \
                and player.rect.left > room.rect.left \
                and player.rect.top >= room.rect.top \
                and player.rect.bottom <= room.rect.bottom if not pl else pl
            pr = player.rect.right + player.rect.width * 2 > room.rect.left \
                and player.rect.right < room.rect.right \
                and player.rect.top >= room.rect.top \
                and player.rect.bottom <= room.rect.bottom if not pr else pr
    return pu, pd, pl, pr
