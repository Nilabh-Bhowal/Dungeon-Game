...
def check_passability(player, current_room, rooms):
    for room in rooms:
        if room != current_room:
            ...
            pr = player.rect.x + 2 * player.rect.width > room.rect.x and player.x < room.rect.x + room.rect.width and player.rect.y >= room.rect.y and player.rect.y + player.rect.height <= room.rect.y + room.rect.height
            if pl:
                print(room)
            return pu, pd, pl, pr
