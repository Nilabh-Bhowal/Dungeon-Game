...
def can_pass(player, current_room, rooms):
    ...
    for room in rooms:
        if room != current_room:
            pu = pu or player.rect.top - player.rect.height * 2 < room.rect.bottom \
                and player.rect.top > room.rect.top \
                and player.rect.left >= room.rect.left \
                and player.rect.right <= room.rect.right
            pd = pd or player.rect.bottom + player.rect.height * 2 > room.rect.top \
                and player.rect.bottom < room.rect.bottom\
                and player.rect.left >= room.rect.left \
                and player.rect.right <= room.rect.right
            pl = pl or player.rect.left - player.rect.width * 2 < room.rect.right \
                and player.rect.left > room.rect.left \
                and player.rect.top >= room.rect.top \
                and player.rect.bottom <= room.rect.bottom
            pr = pr or player.rect.right + player.rect.width * 2 > room.rect.left \
                and player.rect.right < room.rect.right \
                and player.rect.top >= room.rect.top \
                and player.rect.bottom <= room.rect.bottom
    return pu, pd, pl, pr
...
def collide(player, rooms):
    for room in rooms:
        if player.rect.colliderect(room):
            ...
            if not pl and player.rect.left <= room.rect.left + 2.5:
                player.rect.left = room.rect.left + 5.5
            elif not pr and player.rect.right >= room.rect.right - 2.5:
                player.rect.left = room.rect.right - player.rect.width - 5.5

            if not pu and player.rect.top <= room.rect.top + 2.5:
                player.rect.top = room.rect.top + 5.5
            elif not pd and player.rect.bottom >= room.rect.bottom - 2.5:
                player.rect.top = room.rect.bottom - player.rect.height - 5.5
