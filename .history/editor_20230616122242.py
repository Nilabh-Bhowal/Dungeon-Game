...
while running:
    ...
    if pygame.mouse.get_pos()[0] <= 980 and pygame.mouse.get_pressed()[0] and not pressed:
        ...
        if current_item == "room":
            level.append(dungeon.DungeonRoom(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 256) / 3) * 3, round((pygame.mouse.get_pos()[1] + scroll[1] - 256) / 3) * 3))
        elif current_item == "corridor":
            level.append(dungeon.Corridor(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 64) / 3) * 3, round((pygame.mouse.get_pos()[1] + scroll[1] - 32) / 3) * 3))
        elif current_item == "chest":
            level.append(dungeon.Chest(round((pygame.mouse.get_pos()[
                         0] + scroll[0] - 32) / 3) * 3, round((pygame.mouse.get_pos()[1] + scroll[1] - 16) / 3) * 3))
    ...
    for i in range(17):
        pygame.draw.line(screen, (0, 0, 0), (i * 3 -
                         scroll[0] % 3, 0), (i * 3 - scroll[0] % 3, 720))
    for i in range(13):
        pygame.draw.line(screen, (0, 0, 0), (0, i * 3 -
                         scroll[1] % 3), (980, i * 3 - scroll[1] % 3))
