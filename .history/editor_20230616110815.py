...
def check_collision(rect, level, scroll):
    for item in level:
        if rect != item.rect and item.rect.colliderect(rect):
            if rect.bottom > item.rect.top and pygame.mouse.get_pos()[1] + scroll[1] < item.rect.top:
                rect.bottom = item.rect.top
            elif rect.top < item.rect.bottom and pygame.mouse.get_pos()[1] + scroll[1] > item.rect.bottom:
                rect.top = item.rect.bottom
            if rect.right > item.rect.left and pygame.mouse.get_pos()[0] + scroll[0] < item.rect.left:
                rect.right = item.rect.left
            elif rect.left < item.rect.right and pygame.mouse.get_pos()[0] + scroll[0] > item.rect.right:
...
corridor_button = ui.Button("Comic Sans MS", 15, (255, 255, 0), "Corridor", 1000, 300, 200, 50)
