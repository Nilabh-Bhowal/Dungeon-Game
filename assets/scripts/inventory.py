...
class Inventory:
    def __init__(self):
        ...
        self.hotbar = ["empty" for _ in range(4)]
        self.active_slot = 0
    ...
    def handle_mouse_interaction(self, item_carrying, scaled_mouse_pos):
        ...
        for spot, item in enumerate(self.hotbar):
            rect = pygame.Rect(self.x - 200 + spot * 100, self.y, 75, 75)
    ...
    def draw(self, screen, scaled_mouse_pos):
        self.item_carrying = self.handle_mouse_interaction(self.item_carrying, scaled_mouse_pos)
    ...
    def draw_hotbar(self, screen):
        ...
        for spot, item in enumerate(self.hotbar):
            ...
            if spot == self.active_slot:
                ...
                s.fill((127, 127, 127))
            screen.blit(s, (self.x - 200 + spot * 100, self.y, 75, 75))
            pygame.draw.rect(screen, (0, 0, 0), (self.x - 200 + spot * 100, self.y, 75, 75), 5)

            if item != "empty":
                if not isinstance(item, list):
                    screen.blit(self.icons[item], (self.x - 200 + 22 + spot * 100, self.y + 22))
                else:
                    key = item[0]
                    screen.blit(self.icons[key], (self.x - 200 + 22 + spot * 100, self.y + 22))
